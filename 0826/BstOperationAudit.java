...
private void range(ReviewNode node,int low,int high,List<Integer> result){
    if(node == null) return;
...
void forceDeepViolationForTest(){
 ...
 class BstOperationAudit{
    private static void audit(ReviewBst tree,String operation,Supplier<Boolean>action){
        boolean result = action.get();
        System.out.println(operation+","+result+","+tree.size()+","+tree.height()+","+tree.isValid()+","+tree.inorder());
    }
public static void main(String[] args) {
    ReviewBst tree = new ReviewBst();

    audit(tree,"add(10)",() -> tree.add(10));
    audit(tree,"add(50)",() -> tree.add(50));
    audit(tree,"add(20)",() -> tree.add(20));
    audit(tree,"add(30)",() -> tree.add(30));
    audit(tree,"add(70)",() -> tree.add(70));
    
    audit(tree,"add(20)",() -> tree.remove(20));
